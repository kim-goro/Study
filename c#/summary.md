...
# 컬렉션
- 컬렉션 : 스택 메모리, 힙 메모리 사이의 박싱과 언박싱과정이 있다.
## Hashtable
```c#
using System;
...
public static void main(){
    MyDelegate Compare = (a,b) =>{
        if (a>b)
            ...
            Console.WriteLine("{0},{1}은 같다", b, a);
    }
}
```
...
- 코드가 개별적으로 시작과 끝 : 비동기 방식 (Async)
    - one(), two(), three()가 동시에 처리되는 멀티 쓰레딩 프롤그램 구현가능
```c#
    void Start(){
        StartCoroutine(one());
        StartCoroutine(two());
...
## 델리게이트
- 메소드를 대신해서 호출하는 명부를 작성
- 반환, 매개변수를 참조할 메소드에 일치시킴
```c#
public class Calculator : MonoBehavior{
    delegate float Calculate(float a, float b); // 선언
    Calcuate onCalcuate; // 생성

    void Start(){
        onCalculate = Sum; // 메소드 참조
        onCalculate += Subtract; // 메소드 참조
        onCalculate -= Subtract; // 메소드 참조
    }
...
- 상속도 좋은 방법이지만, 클래스가 sealed로 한정되어 있는 경우에는 확장 메소드의 사용을 고려해 볼 수 있습니다.
```c#
using System;
using Extension;
